 ...
 * Home: mismo layout que tienda + botón "Ver en la tienda" en la barra de acciones.
 */
...
<div class="modal-overlay<?= $modal_home ? ' hidden' : '' ?>" id="product-modal" onclick="typeof cerrarModalProducto==='function'?cerrarModalProducto(event):(typeof cerrarModalProductoLanding==='function'&&cerrarModalProductoLanding(event))" role="dialog" aria-modal="true" aria-labelledby="modal-title" aria-hidden="true">
    <div class="product-modal-content flex flex-col md:flex-row gap-0 md:gap-6 p-5 relative bg-white w-full max-w-3xl mx-4 rounded-2xl shadow-2xl custom-scrollbar" onclick="event.stopPropagation()">
        ...
        <div class="w-full md:w-7/12 flex flex-col pt-1 min-h-0">
            ...
            <?php if ($modal_home): ?>
            <div class="improgyp-modal-footer-home mt-auto pt-4 border-t border-slate-100 flex-shrink-0 flex flex-col gap-3">
                <div class="flex items-center justify-end gap-2 flex-shrink-0">
                    <button type="button" id="modal-btn-share" class="w-10 h-10 rounded-lg flex items-center justify-center text-lg transition-colors border border-slate-200 shadow-sm text-slate-400 hover:text-[#1B263B] hover:border-[#1B263B]/30" title="Compartir producto"><i class="fa-solid fa-share-nodes"></i></button>
                    <button type="button" id="modal-btn-wishlist" class="w-10 h-10 rounded-lg flex items-center justify-center text-lg transition-colors border border-slate-200 shadow-sm" title="Lista de deseos"></button>
                    <div id="modal-btn-add-wrapper" class="flex-shrink-0">
                        <button type="button" id="modal-btn-add" class="btn-IMPROGYP px-4 h-10 text-[13px] w-28"><i class="fa-solid fa-cart-plus"></i> <span class="ml-1">Añadir</span></button>
                    </div>
                </div>
                <div class="flex items-end justify-between gap-3 flex-shrink-0">
                    <div>
                        <p class="text-[9px] text-slate-400 font-bold uppercase tracking-widest mb-1">Precio</p>
                        <span id="modal-price" class="text-2xl font-black text-slate-800"></span>
                    </div>
